...
| FR-04 | 회원 | 계정 관리 | 마이페이지, 아이디 찾기, 비밀번호 · 이메일 변경 | ✅ |
...
| FR-13 | 챗봇 | 대화 기록 · 이용 권한 | 회원 세션 · 대화 기록 저장. 질문은 로그인 후 가능, 비로그인은 둘러보기만 | ✅ |
...
- **출발지가 있을 때**: 코스 작성 화면에서 지도에 출발지를 찍으면 출발지 → 1번 → 2번 → 구장 → 경기 후 순서로, **앞 지점 주변(800m → 1.5km → 3km)을 새로 검색**하고 구장 쪽으로 다가가는 곳을 우선합니다. 경기 후 장소는 구장 1.5km 안에서 고릅니다.
...
➡️ 응답 시간의 **99% 이상이 LLM 생성**입니다. 벡터DB는 병목이 아니므로, 속도 개선은 정해진 질문을 DB 결과로 바로 답하기 · 도구 호출 수 제한(최대 4회) · 모델 설정 조정 쪽에서 합니다. 지금의 에이전트 파이프라인은 도구 계획(LLM 1~5회)과 최종 답변(LLM 1회)으로 **LLM을 2~6회** 부르므로 위 수치보다 오래 걸립니다 (스트리밍 적용 전 09-15 로컬 측정 3 ~ 7초). 그래서 **최종 답변은 토큰 스트리밍으로 바로 보여 주고 기다리는 동안은 도구 진행 상황을 표시**합니다.
...
> TC-01 ~ 03 · 07 · 08은 2026-09-15 로컬 서버(에이전트 파이프라인, 토큰 스트리밍 적용 전)에서, TC-04 · 05 · 09 ~ 12는 2026-09-14 골든셋 평가에서 확인한 결과입니다. TC-06 · 13은 dispatcher 규칙(무관 주제 단어 + 야구 단어 없음, 또는 항상 차단하는 주제)을 코드와 단위 테스트로 확인한 결과입니다.
...
python manage.py test llm.test_course_chain              # 11건: 출발지 기준 단계별 코스 · 접두어 분리 · 국외 좌표 무시
