...
- 카카오 지도·장소 검색은 로컬에서 연결을 확인했습니다. 장소 검색은 브라우저에서 `/api/places/search/`로 Django를 직접 호출하며 Next 릴레이를 사용하지 않습니다. 배포 시 JavaScript SDK 도메인을 등록하고 구장 경계 필터를 검증해야 합니다. 설정·카테고리·표시 한도·데이터 한계는 [직접 코스 작성 안내](docs/NEARBY_PLANNER.md)를 참고하세요.
