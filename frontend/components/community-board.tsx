...
export function CommunityBoard({ section, teamCode = "", postId = "" }: { section: keyof typeof boards; teamCode?: string; postId?: string }) {
  ...
  const [editorPost, setEditorPost] = useState<TeamCommunityPost | null | undefined>(undefined);
  ...
  function openEditor(post?: TeamCommunityPost) { setActionError(""); setEditorPost(post ?? null); }
  function closeEditor() { setEditorPost(undefined); }
  function saved(post: TeamCommunityPost) {
    setDetail({ key: `${section}:${post.id}`, post, loading: false, error: "" });
    detailRequest.current = `${section}:${post.id}`;
    closeEditor();
    router.replace(postHref(post.teamCode, post.id));
  }
  ...
  return <main className={`container ${styles.page}`}>
    ...
    {selectedPost && <article className={styles.postDetail}>
      <header className={styles.postHeader}>
        ...
        {owned && <div className={styles.ownerActions}><button type="button" onClick={() => openEditor(selectedPost)} disabled={deleting}>수정</button><button type="button" onClick={() => void removePost()} disabled={deleting}>{deleting ? "삭제 중…" : "삭제"}</button></div>}
        {actionError && <p role="alert">{actionError}</p>}
      </header>
      <div className={`${styles.postBody} ${styles.teamPostBody} ${styles.reportableBody}`}><div>{selectedPost.content}</div><PostReportButton postNumber={selectedPost.postNumber} postId={selectedPost.id} /></div>
      <CommunityPostVote post={selectedPost} />
      <CommunityPostBottom key={selectedPost.id} post={selectedPost} posts={posts} teamCode={team?.code} isFree={section === "free"} onWrite={() => openEditor()} />
    </article>}
    <section aria-label={`${board.title} 글 목록`}>
      <div className={styles.listWriteActions}><button type="button" onClick={() => openEditor()}>글쓰기</button></div>
    ...
    {editorPost !== undefined && <CommunityPostEditor board={section} teamCode={team?.code} post={editorPost ?? undefined} onClose={closeEditor} onSaved={saved} />}
  </main>;
}
