...
import { setPreviewSignedIn, usePreviewMember } from "@/lib/member-preview";
...
export function MemberHeaderActions() {
  ...
  const isAdmin = Boolean(user?.is_superuser || user?.is_staff || previewMember?.role === "master");
  ...
  async function logout() {
    ...
    try {
      ...
      setUser(null);
      router.push("/");
      setError("서버의 로그아웃 여부를 확인하지 못했어요.");
    }
  }
    ...
    {status === "loading" ? <span role="status">회원 확인 중…</span> : user ? <>
      ...
      <div className="member-menu" ref={root}>
        ...
        {open && <nav id="member-menu-panel" className="member-menu-panel" aria-label="마이페이지 메뉴">
          {isAdmin ? <>
            <Link href="/admin" onClick={() => setOpen(false)}>회원 관리</Link>
            <Link href="/admin/posts" onClick={() => setOpen(false)}>게시글 관리</Link>
            <Link href="/admin/reports" onClick={() => setOpen(false)}>신고 목록 관리</Link>
          </> : <>
      ...
      {status === "unavailable" && <span role="status">회원 서버 확인 필요</span>}
    </>}
