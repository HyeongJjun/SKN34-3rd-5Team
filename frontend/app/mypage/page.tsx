...
import { updatePreviewAvatar, updatePreviewProfile, usePreviewMember } from "@/lib/member-preview";
...
function MyPageContent() {
  ...
  const routesReady = useRoutesReady();
  ...
  useEffect(() => {
    if (preview?.role === "master") router.replace("/admin");
  }, [preview?.role, router]);
  ...
  if (preview?.role === "master") return <main className={`container ${styles.page}`}><p role="status">관리자 화면으로 이동하고 있어요.</p></main>;
  if (preview && !routesReady) return <main className={`container ${styles.page}`}><p role="status">회원 화면을 불러오고 있어요.</p></main>;
  ...
  const ownRoutes = routes.filter(route => !route.isSample);
  const likedRoutes = routes.filter(route => likes.includes(route.id));
  const visibleRoutes = tab === "likes" ? likedRoutes : ownRoutes;
  return <main className={`container ${styles.page}`}>
    ...
    <p className={styles.note}>{preview ? "일반 회원 미리보기 · 프로필, 코스와 게시글은 이 브라우저에만 저장돼요." : "계정·프로필 설정은 서버에 저장돼요. 코스·게시글·알림 발송 정책은 아직 회원 계정과 연결되지 않았어요."}</p>
    ...
    {tab === "posts" && preview ? <MemberPosts /> : tab !== "profile" ? preview ? <section aria-label={tab === "likes" ? "찜한 코스" : "내 코스"}>
      {visibleRoutes.length ? <div className={styles.grid}>{visibleRoutes.map(route => <div key={route.id}><RouteCard route={route} />{tab === "courses" && <div className={styles.actions}><Link href={`/routes/new?edit=${encodeURIComponent(route.id)}`}>수정하기</Link><Link href={`/routes/${encodeURIComponent(route.id)}`}>상세 보기</Link></div>}</div>)}</div> : <div className={styles.empty}><h2>{tab === "likes" ? "아직 찜한 코스가 없어요" : tab === "posts" ? "아직 작성한 글이 없어요" : "아직 저장한 코스가 없어요"}</h2><p>{tab === "likes" ? "마음에 드는 코스에 좋아요를 눌러보세요." : "지도에서 장소를 골라 첫 코스를 만들어보세요."}</p><Link className="button button-primary" href={tab === "likes" ? "/routes" : "/routes/new"}>{tab === "likes" ? "코스 둘러보기" : "코스 만들기"}</Link></div>}
    </section> : <section className={styles.empty}><h2>아직 계정 데이터와 연결되지 않았어요</h2><p>이 브라우저의 로컬 코스·게시글을 실회원 소유 데이터로 표시하지 않아요.</p></section> : <section className={styles.settings}><h2>회원정보 수정</h2>
      <ProfilePhotoEditor avatar={user.avatar} onSaved={async avatar => { if (preview) { updatePreviewAvatar(avatar); return; } const updated = await patchUser({ avatar }); setUser(updated, user.id); }} />
