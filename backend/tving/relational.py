...
from .models import (
    TvingPlayer, TvingPlayerCareerRecord, TvingPlayerSeasonRecord,
    TvingScheduleDay, TvingTeamProfile, TvingTeamRoster, TvingTeamSeasonRecord,
    TvingTeamTopPlayer,
)
...
def _ensure_player(code, team, name, now, image_url=None):
    try:
        with transaction.atomic():
            return TvingPlayer.objects.create(external_code=code, team=team, name=name, image_url=image_url, identity_source_fetched_at=now, identity_last_synced_at=now)
    except IntegrityError:
        player = TvingPlayer.objects.select_for_update().get(external_code=code)
...
def _game_values(game, now, raw=False):
    ...
    return day, game_time, team_for(away["code"]), team_for(home["code"]), {
        ...
        "source_status_label": game.get("statusLabel") or game["status"], "source_fetched_at": now, "last_synced_at": now,
...
def persist_daily(data, now):
    ...
    with transaction.atomic():
        ...
        _upsert(TvingScheduleDay, {"date": day}, {"status": "ready" if data["games"] else "empty", "game_count": len(data["games"]), "game_codes": [game["id"] for game in data["games"]]}, now)
        ...
        for kind, rows in (("pitcher", data["individualRankings"]["pitchers"]), ("hitter", data["individualRankings"]["hitters"])):
            for ranking in rows:
                ...
                _upsert(TvingPlayerSeasonRecord, {"player": player, "season": day.year, "record_kind": kind, "record_key": "ranking"}, {
...
def persist_month(data, now):
    with transaction.atomic():
        ...
        for day in data["days"]:
            ...
            _upsert(TvingScheduleDay, {"date": Date.fromisoformat(day["date"])}, {"status": day["status"], "game_count": day["gameCount"], "game_codes": codes}, now)
        for game in data["games"]:
            _upsert_game(game, now, reserved_pk=reserved.get(game["id"]), reserved_else=reserved_pks - {reserved.get(game["id"])})
...
def persist_team(data, now):
    ...
    with transaction.atomic():
        ...
        _upsert(TvingTeamProfile, {"team": team}, {"external_code": data["code"], "short_name": data["shortName"], "image_url": data["teamImageUrl"], "background_image_url": data["backgroundImage"], "season_title": data["seasonTitle"], "roster_codes": roster_codes, "top_keys": top_keys}, now)
        for category, records in (("main", data["mainRecords"]), ("box", data["boxRecords"])):
            for record in records:
                _upsert(TvingTeamSeasonRecord, {"team": team, "season": season, "category": category, "title": record["title"]}, {"value": record["value"]}, now)
        ...
        for position, athletes in data["rosters"].items():
            for athlete in athletes:
                ...
                _upsert(TvingTeamRoster, {"team": team, "player": player}, {"position": position, "back_number": athlete["backNumber"]}, now)
        for athlete_type, groups in data["rankings"].items():
            for group in groups:
                for athlete in group["athletes"]:
                    player = _ensure_player(athlete["code"], team, athlete["name"], now, athlete["imageUrl"])
                    _upsert(TvingTeamTopPlayer, {"team": team, "player": player, "athlete_type": athlete_type, "category": group["title"]}, {"rank": athlete["rank"], "value": athlete["value"], "image_url": athlete["imageUrl"]}, now)
...
def persist_athlete(data, now):
    ...
    with transaction.atomic():
        ...
        for record in data["seasonRecords"]:
            _upsert(TvingPlayerSeasonRecord, {"player": player, "season": season, "record_kind": "detail", "record_key": record["title"]}, {"rank": None, "title": record["title"], "value": record["value"], "rank_label": record["rank"], "is_first_rank": record["isFirstRank"], "metrics": {}, "graphs": record["graphs"]}, now)
        columns = data["careerColumns"]
        for position, row in enumerate(data["careerRows"]):
            _upsert(TvingPlayerCareerRecord, {"player": player, "position": position}, {"season_label": row.get("season", ""), "title": data["careerTitle"], "columns": columns, "metrics": row}, now)
...
def daily_sync_time(day):
    values = list(TvingScheduleDay.objects.filter(date=day).values_list("last_synced_at", flat=True))
    values += list(Game.objects.filter(game_date=day, source="tving").values_list("last_synced_at", flat=True))
    values += list(StandingHistory.objects.filter(snapshot_date=day, source="tving").values_list("last_synced_at", flat=True))
    values += list(TvingPlayerSeasonRecord.objects.filter(season=day.year, record_kind__in=("pitcher", "hitter")).values_list("last_synced_at", flat=True))
    return min((value for value in values if value), default=None)


def month_sync_time(month):
    year, number = int(month[:4]), int(month[5:])
    values = list(TvingScheduleDay.objects.filter(date__year=year, date__month=number).values_list("last_synced_at", flat=True))
    return min((value for value in values if value), default=None)


def team_sync_time(team):
    values = list(TvingTeamProfile.objects.filter(team=team).values_list("last_synced_at", flat=True))
    values += list(TvingTeamRoster.objects.filter(team=team).values_list("last_synced_at", flat=True))
    values += list(TvingTeamTopPlayer.objects.filter(team=team).values_list("last_synced_at", flat=True))
    return min((value for value in values if value), default=None)


def athlete_sync_time(player):
    values = [player.profile_last_synced_at, player.detail_last_synced_at]
    values += list(player.season_records.filter(record_kind="detail").values_list("last_synced_at", flat=True))
    values += list(player.career_records.values_list("last_synced_at", flat=True))
    return min((value for value in values if value), default=None)


def read_daily(day):
    date = Date.fromisoformat(day)
    marker = TvingScheduleDay.objects.filter(date=date).first()
    standings = list(StandingHistory.objects.filter(snapshot_date=date, source="tving").select_related("team").order_by("rank"))
    pitchers = list(TvingPlayerSeasonRecord.objects.filter(season=date.year, record_kind="pitcher").select_related("player__team").order_by("rank"))
    hitters = list(TvingPlayerSeasonRecord.objects.filter(season=date.year, record_kind="hitter").select_related("player__team").order_by("rank"))
...
def read_month(month, today):
    ...
    days = list(TvingScheduleDay.objects.filter(date__year=year, date__month=number).order_by("date"))
...
def read_team(code):
    team = team_for(code)
    profile = TvingTeamProfile.objects.filter(team=team).first()
    if not profile:
        return None
    season = profile.source_fetched_at.astimezone(KST).year
    records = list(TvingTeamSeasonRecord.objects.filter(team=team, season=season).order_by("id"))
    rosters = {position: [] for position in ("pitcher", "infielder", "outfielder", "catcher")}
    for row in TvingTeamRoster.objects.filter(team=team).select_related("player").order_by("id"):
    ...
    top_rows = TvingTeamTopPlayer.objects.filter(team=team).select_related("player").order_by("athlete_type", "category", "rank")
    ...
    profile_images = dict(TvingTeamProfile.objects.values_list("team_id", "image_url"))
...
def read_athlete(code):
    player = TvingPlayer.objects.filter(external_code=code, profile_last_synced_at__isnull=False).select_related("team").first()
    if not player:
        return None
