...
NATURAL_FIELDS = {
    models.Team: ("team_code",), models.Stadium: ("stadium_code",), models.HomeContext: ("season", "team", "stadium"),
    models.PostseasonStage: ("stage_code",), models.Game: ("game_code",), models.StandingHistory: ("team", "snapshot_date"),
    models.SeatZone: ("home_context", "zone_code"), models.TicketPolicy: ("policy_code", "channel_no"),
    models.TicketPrice: ("seat_zone", "price_tier", "day_type", "customer_type", "group_size", "price_krw", "valid_from", "valid_to", "discount_condition"),
    models.SeatMap: ("home_context", "map_title"), models.SeatMapAsset: ("seat_map", "asset_no"),
    models.SeatScope: ("home_context", "scope_code"), models.SeatView: ("seat_scope",),
    models.Transport: ("stadium", "access_code"), models.FoodStore: ("record_code",),
    models.FoodStoreLocation: ("food_store", "location_no"), models.FoodStoreMenu: ("food_store", "menu_category_official"),
    models.StadiumContent: ("record_code",), models.Facility: ("record_code",),
}
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not (root / "raw/team_stadium_code_map.csv").is_file():
            raise CommandError(f"데이터 디렉터리를 찾을 수 없습니다: {root}")
        self.root = root
        self.counts = Counter()
        self.sources = {}
        self.rejections = []
        ...
        with transaction.atomic():
            try:
                ...
                failure = error
            rolled_back = options["dry_run"] or failure is not None or bool(self.rejections)
            if rolled_back:
        ...
        report = {
            ...
            "totals": {
                "source": sum(self.sources.values()),
                "imported": imported,
                "skipped": skipped,
                "rejected": len(self.rejections),
                "persisted": 0 if rolled_back else imported,
            },
        ...
        if self.rejections:
            raise CommandError(f"{len(self.rejections)}개 행이 거부되어 전체 적재를 롤백했습니다")
    ...
    def add(self, model, key, **values):
        ...
        natural = {name: values[name] for name in NATURAL_FIELDS.get(model, ())}
        existing = model.objects.filter(**natural).first() if natural else None
        if existing:
            self.counts[f"{model.__name__}.skipped"] += 1
            return existing
        occupant = model.objects.filter(pk=pk).first()
        if occupant:
            raise CommandError(f"stable ID collision: {model.__name__}:{key}:{pk}")
        obj = model(id=pk, **values)
        try:
            with transaction.atomic():
                # 원본의 명시적 공란은 DB TextField가 표현 가능한 공란으로 보존한다.
                empty_text = [field.name for field in model._meta.fields if field.get_internal_type() in {"TextField", "CharField"} and getattr(obj, field.name, None) == ""]
                obj.full_clean(exclude=empty_text)
