...
> 다이어그램 목록 · README/PPT 사용법: [`docs/architecture/README.md`](../architecture/README.md) · 원본 정의: [`system_architecture.archify.json`](../architecture/system_architecture.archify.json)

---

## 1. 전체 구조

<p align="center"><a href="../architecture/system_architecture.html"><img src="../images/system_architecture.gif" width="100%" alt="시스템 아키텍처 (움직이는 그림)"/></a></p>

<sub>▶ 선을 따라 흐름이 움직입니다 · 정지 그림 <a href="../images/system_architecture.png">PNG</a> · <a href="../images/system_architecture.svg">SVG</a> · PPT용 영상 <a href="../media/system_architecture.mp4">MP4</a> · 클릭하면 확대 · 단계별 설명이 되는 <a href="../architecture/system_architecture.html">인터랙티브 HTML</a></sub>
...
| LLM | RAG 파이프라인 · 에이전트 | LangChain 1.x (`create_agent`) · langchain-openai | – |
...
<p align="center"><a href="../architecture/deploy_cicd.html"><img src="../images/deploy_cicd.gif" width="95%" alt="협업 · CI/CD 배포 흐름 (움직이는 그림)"/></a></p>

<sub>▶ 선을 따라 흐름이 움직입니다 · 정지 그림 <a href="../images/deploy_cicd.png">PNG</a> · <a href="../images/deploy_cicd.svg">SVG</a> · PPT용 영상 <a href="../media/deploy_cicd.mp4">MP4</a> · 클릭하면 확대 · 단계별 설명이 되는 <a href="../architecture/deploy_cicd.html">인터랙티브 HTML</a></sub>
...
| 하는 일 | 파일 로딩 → 청크 텍스트 생성 → 임베딩 → pgvector 적재 | 질문 임베딩 → 검색 → 프롬프트 → 에이전트 생성 |
| 코드 | `llm/management/commands/build_index.py` | `llm/rag/pipeline.py` → `dispatcher.py` → `assistant/pipeline.py` |
| 비용 | 전량 약 100원 (임베딩) | 임베딩 1회 + LLM 1~5회 |
| 시간 | 수 분 | 검색 약 10 ms + 생성 수 초 |

<p align="center"><img src="../images/arch_index_vs_serve.png" width="90%" alt="인덱싱과 서빙 경로"/></p>

<sub>원본: <code>docs/diagrams/arch_index_vs_serve.mmd</code> · 다시 그리기: <code>bash docs/diagrams/render.sh</code></sub>
...
<p align="center"><img src="../images/arch_request_sequence.png" width="90%" alt="요청 처리 시퀀스"/></p>

<sub>원본: <code>docs/diagrams/arch_request_sequence.mmd</code> · 다시 그리기: <code>bash docs/diagrams/render.sh</code></sub>
...
| 에이전트 예외 · 빈 답 | 같은 질문을 기존 도메인(club · venue · course · nearby)으로 다시 답함, `route`에 `agent:error>` 기록 |
...
<sub>원본: <code>docs/diagrams/arch_erd.mmd</code> · 다시 그리기: <code>bash docs/diagrams/render.sh</code></sub>
...
| 회원 채팅 메시지 (SSE) | `POST /api/chat/sessions/{id}/messages/` | JWT · RAG 체인 연결 |
| 답변 확정 (중단 처리) | `POST /api/chat/turns/{turn_id}/finalize/` | JWT |
| 게스트 채팅 | `POST /api/chat/guest/` | 로그인 없이 RAG 체인 사용 |
